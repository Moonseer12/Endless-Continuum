...
class MyrdengrassVines : ModTile
{
    public override string Texture => ECAssets.TilesPath + "MyrdengrassVines";
    public override void SetStaticDefaults() => QuickTile.QuickVineTile(this, DustID.t_Honey, new Color(209, 179, 73));
    ...
    public override bool TileFrame(int i, int j, ref bool resetFrame, ref bool noBreak)
    {
        ...
        if (type == ModContent.TileType<MyrdengrassTile>() || type == Type)
            return true;
        ...
        if (WorldGen.genRand.NextBool(15) && !tileBelow.HasTile && tileBelow.LiquidType != LiquidID.Lava)
        {
            ...
            while (yTest > j - 10)
            {
                ...
                if (testTile.Slope != SlopeType.Solid)
                    break;
                else if (!testTile.HasTile || testTile.TileType != ModContent.TileType<MyrdengrassTile>())
                {
                    yTest--;
                    continue;
                }
    ...
    public override bool PreDraw(int i, int j, SpriteBatch spriteBatch)
    {
        ...
        float xOff = GetOffset(i, j, tile.TileFrameX);
        Vector2 drawPos = (new Vector2(i, j) * 16) - Main.screenPosition;
        Color col = Lighting.GetColor(i, j, Color.White);
        Vector2 zero = Main.drawToScreen ? Vector2.Zero : new Vector2(Main.offScreenRange, Main.offScreenRange);
        spriteBatch.Draw(ModContent.Request<Texture2D>("EndlessContinuum/Assets/Textures/Tiles/MyrdengrassVines").Value, drawPos + zero - new Vector2(xOff, 0), realSource, new Color(col.R, col.G, col.B, 255), 0f, new Vector2(0, 0), 1f, SpriteEffects.None, 0f);
        return false;
    }
